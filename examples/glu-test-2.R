...
# Passing input_shape etc. as '...'

layer_glu_1 <- new_layer_class(
...
# Passing input_shape after shape normalization, all the rest as '...'
...
# Normalizing all the shape-related args

layer_glu_3 <- new_layer_class(
...
model <-
  keras_model_sequential() %>%
  layer_glu_3(10, input_shape = 30)

model

inp  <- layer_input(30)
out  <- layer_glu_3(units = 10, return_gate = TRUE)(inp)
